"""TP1 break-even A/B for final Scan 11 profiles."""
...
from scan11_sat_robustness import load_frames
from bb_squeeze_entry_stage import initial_stop, metrics, bounds_for, COMMISSION_BPS, SLIPPAGE_BPS

FINAL={
 '4H':('bb_tighten',dict(stop_mode='swing',stop_atr=1.2,swing=9,buffer=.25,trail=1.8,max_hold=40,tp1=1.1,tp2=2.2,tp3=3.3,tight=1.0)),
 '1D':('bb_tighten',dict(stop_mode='swing',stop_atr=1.25,swing=7,buffer=.25,trail=2.0,max_hold=60,tp1=1.1,tp2=2.2,tp3=3.3,tight=1.0)),
 '1W':('structural',dict(stop_mode='swing',stop_atr=1.3,swing=11,buffer=.20,trail=2.3,max_hold=80,tp1=1.1,tp2=2.2,tp3=3.3,tight=1.8)),
}
...
def sim(d,sp,p,family,mode):
    ...
    for pos in range(ep,last+1):
        ...
        if family=='bb_tighten':
            basis=float(row['bb_basis']) if pd.notna(row['bb_basis']) else np.nan
            if np.isfinite(basis) and cl<basis:tight=True
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--db',required=True);ap.add_argument('--period',choices=['4H','1D','1W'],required=True);ap.add_argument('--output',required=True);a=ap.parse_args()
    fam,p=FINAL[a.period];frames,times,count=load_frames(a.db,a.period);b=bounds_for(times);rows=[ev(frames,p,fam,b,m) for m in ('none','entry','cost')]
    no=rows[0];ne=float(no['stitched'].get('expectancy_r',0));alts=rows[1:];best=max(alts,key=lambda x:(x['positive_folds'],x['worst_fold_expectancy_r'],float(x['stitched'].get('expectancy_r',0))))
    be=float(best['stitched'].get('expectancy_r',0));decision='NO_BE' if be < ne-0.005 else ('USE_'+best['mode'].upper()+'_BE')
    d={'version':'scan11-be-ab-v1','period':a.period,'entry_frozen':'B_bb_reclaim_with_momentum_confirm','family':fam,'profile':p,'signal_count':count,'rows':rows,'decision':decision,'note':'Human materiality gate remains authoritative if a micro-improvement sacrifices expectancy.'}
