...
def simulate_profile(data: pd.DataFrame, signal_pos: int, profile: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for pos in range(entry_pos, last_pos + 1):
        ...
        if bool(profile.get("technical_exit")) and remaining > 1e-12:
            ...
            if np.isfinite(macd) and np.isfinite(macd_signal) and np.isfinite(ema13) and close < ema13 and macd < macd_signal:
...
def _collect_trades(frames, profile, *, start_time, end_time):
    trades = []
...
def _evaluate_profiles(frames, profiles, *, start_time, end_time, min_trades):
    rows = []
...
def _select_with_validation(frames, profiles, *, train_start, validation_start, holdout_start, min_train_trades, min_validation_trades, top_train):
    train_rows = _evaluate_profiles(frames, profiles, start_time=train_start, end_time=validation_start, min_trades=min_train_trades)
    candidates = train_rows[: min(top_train, len(train_rows))]
    validation_rows = []
    for row in candidates:
        trades = _collect_trades(frames, row["profile"], start_time=validation_start, end_time=holdout_start)
        m = metrics(trades)
    ...
    audit = {"train_top": train_rows[:10], "validation_candidates": validation_rows, "selected": selected}
...
def analyze(database: str, period: str) -> dict[str, Any]:
    ...
    with MarketDataStore(database, read_only=True) as store:
        ...
        for number, symbol in enumerate(symbols, start=1):
            ...
            positions = [int(pos) for pos in np.flatnonzero(data["signal"].to_numpy(dtype=bool)) if pos >= WARMUP and pos + 1 < len(data)]
            if positions:
    ...
    if period == "1M" and len(dev_times) < MIN_MONTHLY_DEVELOPMENT_SIGNALS:
        ...
        validation_trades = _collect_trades(frames, baseline, start_time=validation_start, end_time=holdout_start)
        ...
        return {
            ...
            "split": {"signals_total": len(signal_times), "signals_development": len(dev_times), "validation_start": validation_start.isoformat(), "holdout_start": holdout_start.isoformat()},
    ...
    selected_a, audit_a = _select_with_validation(
        frames, stage_a_profiles, train_start=None, validation_start=validation_start,
        holdout_start=holdout_start, min_train_trades=train_estimate,
        min_validation_trades=validation_estimate, top_train=7,
    )
    ...
    selected_b, audit_b = _select_with_validation(
        frames, stage_b_profiles, train_start=None, validation_start=validation_start,
        holdout_start=holdout_start, min_train_trades=train_estimate,
        min_validation_trades=validation_estimate, top_train=10,
    )
    ...
    for technical in (False, True):
        ...
        validation_trades = _collect_trades(frames, p, start_time=validation_start, end_time=holdout_start)
    ...
    validation_trades = _collect_trades(frames, final_profile, start_time=validation_start, end_time=holdout_start)
    holdout_trades = _collect_trades(frames, final_profile, start_time=holdout_start, end_time=None)
