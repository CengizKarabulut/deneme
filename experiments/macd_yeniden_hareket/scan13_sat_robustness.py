"""SAT robustness for Scan 13 MACD YenidenHareket.

Frozen entry candidates by timeframe:
- 4H: compare A original cross vs C histogram reacceleration under the same exit families
- 1D: C histogram reacceleration
- 1W: A original positive-MACD bullish cross
- 1M: A original, research-only; no production claim from historical sample
"""
...
from scan13_entry_abc import indicators, entry_events  # noqa: E402
...
def simulate(d: pd.DataFrame, sp: int, p: dict[str, Any], family: str) -> dict[str, Any] | None:
    ...
    for pos in range(ep, last+1):
        ...
        below_zero = np.isfinite(m) and m < 0.0

        if family in {"macd_tighten", "adaptive"} and weak:
            tight = True

        hard = False
        if family == "macd_hard":
            hard = weak
        elif family == "zero_hard":
            hard = below_zero
...
def load_frames(db: str, period: str, entry: dict[str, str]):
    frames = []
...
def analyze_entry(db: str, period: str, entry: dict[str, str]):
    ...
    families = ["structural", "macd_tighten", "macd_hard", "zero_hard", "adaptive"]
    fam = [evaluate(frames, b, dict(base), f) for f in families]
    fam.sort(key=score_key, reverse=True)
    winner = fam[0]["family"]

    # 1M is diagnostic only: do not optimize geometry on the small/uneven sample.
    candidates = []
    if period != "1M":
        candidates = [evaluate(frames, b, p, winner) for p in compact_neighbors(period, base)]
        if winner in {"macd_tighten", "adaptive"}:
            for x in sorted(set([max(.8, BASE_TIGHT[period]-.2), BASE_TIGHT[period], BASE_TIGHT[period]+.2])):
