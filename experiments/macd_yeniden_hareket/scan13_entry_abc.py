"""Scan 13 MACD YenidenHareket A/B/C entry research."""
...
def entry_variants() -> list[dict[str, Any]]:
    return [
        {"name": "A_original_positive_macd_cross", "mode": "A"},
        {"name": "B_positive_macd_and_signal_cross", "mode": "B"},
        {"name": "C_positive_hist_reacceleration", "mode": "C"},
    ]


def entry_events(d: pd.DataFrame, variant: dict[str, Any]) -> pd.Series:
    m = pd.to_numeric(d["macd"], errors="coerce")
    s = pd.to_numeric(d["macd_signal"], errors="coerce")
    if variant["mode"] == "A":
        cond = (m > 0.0) & d["macd_cross_up"]
    elif variant["mode"] == "B":
        cond = (m > 0.0) & (s > 0.0) & d["macd_cross_up"]
    elif variant["mode"] == "C":
        cond = d["reaccel_event"]
    else:
...
def analyze(db: str, period: str) -> dict[str, Any]:
    ...
    for variant in variants:
        ...
        if not b:
            rows.append({"entry": variant, "signal_count": counts[name], "folds": [], "stitched": {"trades": 0, "profit_factor": None, "expectancy_r": 0.0, "win_rate_pct": 0.0}, "positive_folds": 0, "worst_fold_expectancy_r": 0.0, "median_fold_expectancy_r": 0.0, "classification": "INSUFFICIENT_SAMPLE"})
            continue
    ...
    rows.sort(key=lambda x: (rank[x["classification"]], x["positive_folds"], x["worst_fold_expectancy_r"], x["median_fold_expectancy_r"], float(x["stitched"].get("expectancy_r", 0.0))), reverse=True)
    return {
        "version": "scan13-entry-abc-v1",
        "period": period,
        "method": "A/B/C only; same structural exit; entry-specific four chronological windows; costs included",
        "definitions": {
            "A": "MACD Level>0 + fresh bullish MACD/Signal crossover",
            "B": "MACD Level>0 + Signal>0 + fresh bullish MACD/Signal crossover",
            "C": "MACD Level>0 + MACD>Signal + histogram rising two bars; fresh episode start only",
        },
        "rows": rows,
