"""TP1 break-even A/B for final Scan 14 MACD DipDonusu candidates."""
...
from scan14_sat_robustness import load_frames  # noqa: E402

_BB=Path(__file__).resolve().parents[1]/'bb_squeeze'
sys.path.insert(0,str(_BB))
from bb_squeeze_entry_stage import initial_stop, metrics, bounds_for, COMMISSION_BPS, SLIPPAGE_BPS  # noqa: E402

ENTRY={
 '4H':{"name":"C_strong_rvol_1_50","mode":"C","rvol_min":1.50},
 '1D':{"name":"A_no_rvol","mode":"A","rvol_min":None},
 '1W':{"name":"A_no_rvol","mode":"A","rvol_min":None},
 '1M':{"name":"A_no_rvol","mode":"A","rvol_min":None},
}
FINAL={
 '4H':('fail_tighten',dict(stop_mode='swing',stop_atr=1.2,swing=7,buffer=.20,trail=2.0,max_hold=40,tp1=1.0,tp2=2.0,tp3=3.0,tight=.8)),
 '1D':('fail_tighten',dict(stop_mode='swing',stop_atr=1.25,swing=7,buffer=.20,trail=2.2,max_hold=40,tp1=1.0,tp2=2.0,tp3=3.0,tight=.8)),
 '1W':('fail_tighten',dict(stop_mode='swing',stop_atr=1.3,swing=11,buffer=.15,trail=2.3,max_hold=60,tp1=1.1,tp2=2.2,tp3=3.3,tight=1.2)),
...
def sim(d,sp,p,family,mode):
    ...
    for pos in range(ep,last+1):
        ...
        if family=='fail_tighten' and np.isfinite(m) and np.isfinite(s) and m<s and m<0:
            tight=True
...
def main():
    ...
    d={'version':'scan14-be-ab-v1','period':a.period,'entry_frozen':entry,'family':fam,'profile':p,'signal_count':count,'rows':rows,'decision':decision,'note':'Human materiality gate remains authoritative; 1M is research-only.'}
