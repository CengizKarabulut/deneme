...
INTRADAY_MINUTES: Dict[str, int] = {
    "15m": 15, "30m": 30, "45m": 45, "1H": 60, "2H": 120, "4H": 240,
}

def normalize_period(period: str) -> str:
    value = str(period).strip()
    mapping = {"15M":"15m","30M":"30m","45M":"45m","1h":"1H","2h":"2H","4h":"4H","1d":"1D","1w":"1W","1m":"1M"}
...
def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    missing = [c for c in OHLCV_COLUMNS if c not in clean.columns]
    if missing:
...
def resample_bist_intraday(df_15m: pd.DataFrame, target_period: str) -> pd.DataFrame:
    ...
    for timestamp in clean.index:
        ...
        elapsed = max(0, int((timestamp-session_start).total_seconds()//60))
        buckets.append(session_start + pd.Timedelta(minutes=(elapsed//minutes)*minutes))
...
def resample_calendar(df_daily: pd.DataFrame, target_period: str) -> pd.DataFrame:
    ...
    if normalize_period(target_period) == "1W":
        buckets = naive.to_period("W-SUN").start_time
    elif normalize_period(target_period) == "1M":
        buckets = naive.to_period("M").start_time
    else:
...
class MarketDataStore:
    ...
    def _create_schema(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS candles (
          symbol TEXT NOT NULL, exchange TEXT NOT NULL, period TEXT NOT NULL,
          candle_time TEXT NOT NULL, open REAL NOT NULL, high REAL NOT NULL,
          low REAL NOT NULL, close REAL NOT NULL, volume REAL,
          PRIMARY KEY (symbol, exchange, period, candle_time));
        CREATE INDEX IF NOT EXISTS idx_candles_lookup ON candles(symbol, exchange, period, candle_time);
    ...
    def set_metadata(self, key: str, value) -> None:
        ...
        self.connection.execute("INSERT INTO metadata(key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, encoded))
        self.connection.commit()
    ...
    def retain_symbols(self, exchange: str, allowed_symbols: Iterable[str]) -> int:
        ...
        with self.connection:
            cursor = self.connection.execute(f"DELETE FROM candles WHERE exchange=? AND UPPER(symbol) NOT IN ({placeholders})", [str(exchange), *allowed])
        return int(cursor.rowcount if cursor.rowcount is not None and cursor.rowcount >= 0 else 0)
    def upsert_dataframe(self, symbol: str, exchange: str, period: str, df: pd.DataFrame, max_bars: Optional[int] = None) -> int:
        if self.read_only:
            raise RuntimeError("Salt okunur veri deposu guncellenemez")
        ...
        rows = [(str(symbol),str(exchange),normalized,pd.Timestamp(ts).isoformat(),float(r.open),float(r.high),float(r.low),float(r.close),None if pd.isna(r.volume) else float(r.volume)) for ts,r in clean.iterrows()]
        with self.connection:
            self.connection.executemany("""INSERT INTO candles(symbol,exchange,period,candle_time,open,high,low,close,volume)
            VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(symbol,exchange,period,candle_time) DO UPDATE SET
            open=excluded.open,high=excluded.high,low=excluded.low,close=excluded.close,volume=excluded.volume""", rows)
        return len(rows)
    def load_dataframe(self, symbol: str, exchange: str, period: str, limit: Optional[int] = None) -> Optional[pd.DataFrame]:
        normalized = normalize_period(period)
        params = [symbol, exchange, normalized]
        if limit and limit > 0:
            query = """SELECT candle_time,open,high,low,close,volume FROM
            (SELECT candle_time,open,high,low,close,volume FROM candles WHERE symbol=? AND exchange=? AND period=? ORDER BY candle_time DESC LIMIT ?)
            ORDER BY candle_time ASC"""
            params.append(int(limit))
        else:
            query = "SELECT candle_time,open,high,low,close,volume FROM candles WHERE symbol=? AND exchange=? AND period=? ORDER BY candle_time ASC"
    ...
    def list_symbols(self, exchange: str, period: str) -> list[str]:
        rows = self.connection.execute("SELECT DISTINCT symbol FROM candles WHERE exchange=? AND period=? ORDER BY symbol", (exchange, normalize_period(period))).fetchall()
        return [str(r[0]) for r in rows]
    def symbol_count(self, exchange: str, period: str) -> int:
        row = self.connection.execute("SELECT COUNT(DISTINCT symbol) FROM candles WHERE exchange=? AND period=?", (exchange, normalize_period(period))).fetchone()
        return int(row[0] or 0)
    def integrity_ok(self) -> bool:
        row = self.connection.execute("PRAGMA integrity_check").fetchone()
        return bool(row and row[0] == "ok")
    def build_manifest(self) -> dict:
        rows = self.connection.execute("SELECT exchange,period,COUNT(DISTINCT symbol),COUNT(*),MAX(candle_time) FROM candles GROUP BY exchange,period ORDER BY exchange,period").fetchall()
        return {"generated_at": datetime.now(TZ_TURKEY).isoformat(timespec="seconds"), "database": os.path.basename(self.path), "integrity_ok": self.integrity_ok(), "datasets":[{"exchange":e,"period":p,"symbols":s,"rows":c,"latest_candle":l} for e,p,s,c,l in rows]}
